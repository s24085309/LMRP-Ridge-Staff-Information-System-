...
import { learnerImportAction, type LearnerImportState } from "@/app/(app)/s3/admin/learners/import/actions";
import { LEARNER_IMPORT_COLUMNS } from "@/lib/s3/learner-import";
import { csvEscapeField } from "@/lib/csv";

const initialState: LearnerImportState = { stage: "idle" };
...
export default function LearnerImportForm() {
  const [state, formAction, pending] = useActionState(learnerImportAction, initialState);
    ...
    return (
      <div className="rounded-xl border border-ro-teal-500/20 bg-ro-teal-500/5 p-6">
        ...
        <p className="mt-2 text-sm text-slate-300">
          Imported: <strong>{state.result.imported}</strong> new learners · Updated:{" "}
          <strong>{state.result.updated}</strong> existing learners.
        </p>
        <Link href="/s3/admin/learners" className="ro-focus-ring mt-4 inline-block rounded-lg bg-ro-teal-500 px-4 py-2 text-sm font-semibold text-ro-navy-950 hover:brightness-110">
          View Learners
        </Link>
      </div>
    );
    ...
    return (
      <div>
            ...
            <tbody>
              {preview.rows.map((r) => (
                <tr key={r.rowNumber} className="border-t border-white/5 text-slate-200">
                  ...
                  <td className="px-3 py-1.5">{r.learnerId}</td>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {errorRows.length > 0 && (
            <DownloadErrorsButton rows={errorRows} />
          )}

          <Link href="/s3/admin/learners/import" className="ro-focus-ring text-sm text-slate-400 hover:text-white">
            Start over
          </Link>
        </div>
      </div>
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="rounded-lg border border-white/10 bg-ro-navy-900 p-4 text-sm text-slate-300">
        ...
        <p className="mt-1 text-xs text-slate-400">{LEARNER_IMPORT_COLUMNS.join(", ")}</p>
        <p className="mt-2 text-xs text-slate-500">
          Save your spreadsheet as CSV first (File → Save As → CSV in Excel/Sheets).
        </p>
      </div>
...
function DownloadErrorsButton({
  ...
  rows: { rowNumber: number; learnerId: string; firstName: string; surname: string; errors: string[] }[];
}) {
  const download = () => {
    const header = ["Row", "Learner ID", "First Name", "Surname", "Errors"];
    const lines = [header, ...rows.map((r) => [String(r.rowNumber), r.learnerId, r.firstName, r.surname, r.errors.join("; ")])];
